                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-4 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
                        <p class="text-sm text-blue-700 dark:text-blue-300">
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ auth()->user()->role === 'admin' ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' }}">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($spareparts as $index => $sparepart)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $sparepart->stok > 10 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : ($sparepart->stok > 0 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200') }}">
                                                {{ $sparepart->stok }} unit
                                            </span>
                                        </td>
                                        ...
                                        @if(auth()->user()->role === 'admin')
                                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                                <div class="flex justify-center space-x-2">
                                                    ...
                                                    <form action="{{ route('spareparts.destroy', $sparepart) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                    ...
                                    <tr>
                                        <td colspan="{{ auth()->user()->role === 'admin' ? '7' : '6' }}" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Belum ada data sparepart.</p>
                                                @if(auth()->user()->role === 'admin')
                                                    <a href="{{ route('spareparts.create') }}" class="mt-3 text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 text-sm font-medium">
                                                        + Tambah Data Pertama
                                                    </a>
                                                @endif
                                            </div>
